...
public class EditOperationsManager {
  ...
  private int buildRoad(Node newNode, Node lastNode) {
    if (newNode.getRoadPair().size() >= ROAD_LIMIT || lastNode.getRoadPair().size() >= ROAD_LIMIT) {
      return -1;
    }
   ...
   * @return: 0 - success. -1 - too many roads in the node. -2 - road places on the existing road.
   */
  ...
  public void updateCongestions(ReportWindowStruct reportWindowStruct) {
    for (int i = 0;
        i < Math.min(reportWindowStruct.getCongestionList().size(), map.getRoadCount());
        i++) {
  ...
  public void applyTrafficLightSettings(Node node, int greenDelay, int redDelay) {
