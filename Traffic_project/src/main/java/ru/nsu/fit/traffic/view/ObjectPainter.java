...
public class ObjectPainter {
  ...
  private double getAngle(double machineX, double machineY, double destX, double destY) {
    // Получим косинус угла по формуле
    double cos = (machineX * destX + machineY * destY) / (Math.sqrt(machineX * machineX + machineY * machineY)
      * Math.sqrt(destX * destX + destY * destY));
  ...
  public Shape paintCar(CarState carState, Road road, Lane lane) {
    Rectangle car = new Rectangle(
      (int) Math.round((road.getFrom().getX() + road.getTo().getX()) / 2) + (LANE_SIZE * carState.getCurrentLane()),
      (int) Math.round((road.getFrom().getY() + road.getTo().getY()) / 2) + (LANE_SIZE * carState.getCurrentLane()),
      LANE_SIZE - 4, 10
    );
    System.out.println(getAngle(
      (int) Math.round((road.getFrom().getX() + road.getTo().getX()) / 2),
      (int) Math.round((road.getFrom().getY() + road.getTo().getY()) / 2),
      road.getTo().getX(), road.getTo().getY()));
    car.setRotate(getAngle(car.getX(), car.getY(), road.getTo().getX(), road.getTo().getY()));
